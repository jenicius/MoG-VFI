...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                        unconditional_guidance_scale=1.0, cfg_img=None, fs=None, text_input=False, multiple_cond_cfg=False, loop=False, interp=False, timestep_spacing='uniform', guidance_rescale=0.0, vfi=None, **kwargs):
    ...
    img = videos[:,:,0] # bchw
    img_emb = model.embedder(img) ## blc 768
    img_emb = model.image_proj_model(img_emb) # b * 16 * 1024
    
    motion = cal_flow(videos, model.vfi)

    cond_emb = model.get_learned_conditioning(prompts) # b * 77 * 1024
    ...
    if model.model.conditioning_key == 'hybrid':
        ...
        cond["c_concat"] = [img_cat_cond] # b c t h w intermediate frames are zero
    ...
    if unconditional_guidance_scale != 1.0:
        ...
        uc["motion_guidance"] = cond["motion_guidance"]
        if model.model.conditioning_key == 'hybrid':
            uc["c_concat"] = [img_cat_cond]
    else:
        uc = None

    additional_decode_kwargs = {'ref_context': hs}#{'ref_context': hs}
...
def get_parser():
    ...
    parser.add_argument("--height", type=int, default=540, help="image height, in pixel space")
    parser.add_argument("--width", type=int, default=960, help="image width, in pixel space")
