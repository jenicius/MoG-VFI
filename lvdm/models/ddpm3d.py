...
import math
from emavfi.vfi_utils import get_vfi_model, cal_flow, warp_fea
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        if self.noise_strength > 0: # 0
            ...
            noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise) # 加噪 xt = f(x0, t)
        # print(cond.keys())
        model_output = self.apply_model(x_noisy, t, cond, **kwargs) # 预测 
        ...
        if self.parameterization == "x0":
            ...
            target = self.get_v(x_start, noise, t) #a*eps - sigma* x0
        else:
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        if ddim:
            ...
            samples, intermediates = ddim_sampler.sample(ddim_steps, batch_size, shape, cond, verbose=False, func=None, **kwargs)
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        # 从get batch得到输入的z cond 重建xrec，然后从
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):
        ## x: b c t h w 获得batch的视频输入
        x = super().get_input(batch, self.first_stage_key)
        ## encode video frames x to z via a 2D encoder   通过encoder获得2d的特征 然后映射到2*emd 为什么有2呢 因为代表均值和方差 就是说每次采样都是不一样的latent     
        z = self.encode_first_stage(x)
        
        ## get caption condition 获得caption的list
        cond_input = batch[self.cond_stage_key]

        if isinstance(cond_input, dict) or isinstance(cond_input, list):
            cond_emb = self.get_learned_conditioning(cond_input) # 通过CLIP的text encoding来运行
        else:
            cond_emb = self.get_learned_conditioning(cond_input.to(self.device))
                
        cond = {}
        ## to support classifier-free guidance, randomly drop out only text conditioning 5%, only image conditioning 5%, and both 5%.
        if random_uncond:  # 0.05 
        ...
        img = x[:,:,cond_frame_index,...] # B C T H W
        img = input_mask * img 
        ## img: b c h w
        img_emb = self.embedder(img) ## b l c 
        img_emb = self.image_proj_model(img_emb) # qformer 

        motion = cal_flow(x, self.vfi)
        
        if self.model.conditioning_key: 
            z_inter = warp_fea(motion, z)
            img_cat_cond = z.detach().clone()
            img_cat_cond[:, :, 1:-1] = z_inter
            cond["c_concat"] = [img_cat_cond] # b c t h w
        cond["c_crossattn"] = [torch.cat([prompt_imb, img_emb], dim=1)] ## concat in the seq_len dim
        cond["motion_guidance"] = motion
    ...
    @torch.no_grad()
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=True, \
        ...
        if sample:
            ...
            if unconditional_guidance_scale != 1.0:
                ...
                ## img: b c h w
                img_emb = self.embedder(img,1) ## b l c
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, motion_guidance: list = None, 
        ...
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, motion=motion_guidance, **kwargs)
