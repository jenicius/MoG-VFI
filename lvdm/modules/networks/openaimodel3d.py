...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, batch_size=None, motion=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb, batch_size=batch_size)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, bs=batch_size)
            elif isinstance(layer, TemporalTransformer):
                x = rearrange(x, '(b f) c h w -> b c f h w', b=batch_size)
                x = layer(x, context, motion)
...
class UNetModel(nn.Module):
        ...
        if self.addition_attention:
            self.init_attn=TimestepEmbedSequential(
                TemporalTransformer(
                    ...
                    temporal_length=temporal_length,fuse=True))
    ...
    def forward(self, x, timesteps, context=None, motion=None, features_adapter=None, fs=None, **kwargs):
        b,_,t,_,_ = x.shape
        ...
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=context, batch_size=b, motion=motion)
            if id ==0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context, batch_size=b, motion=motion)
        ...
        h = self.middle_block(h, emb, context=context, batch_size=b, motion=motion)
