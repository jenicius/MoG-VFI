...
from emavfi.vfi_utils import warp_fea
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context=None, mask=None, context2=None):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else context2, mask=mask) + x
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 ...
                 relative_position=False, temporal_length=None, fuse=False):
        super().__init__()
    ...
    def forward(self, x, context=None, motion=None):
        b, c, t, h, w = x.shape
        x_in = x
        x = self.norm(x)
        ...
        # warp 
        if self.fuse:
            x_w = rearrange(x, '(b h w) t c -> b c t h w', b=b, h=h, w=w).contiguous()
            x_m_w = warp_fea(motion, x_w)
            x_w[:, :, 1:-1] = x_m_w
            x_w = rearrange(x_w, 'b c t h w -> (b h w) t c').contiguous()
            
            for i, block in enumerate(self.transformer_blocks):
                x_w = block(x_w, mask=mask)
        ...
        if self.fuse:
            x = rearrange(x, 'b hw t c -> (b hw) t c').contiguous()
            x = (x + x_w) * 0.5
            x = rearrange(x, '(b hw) t c -> b hw t c', b=b).contiguous()
